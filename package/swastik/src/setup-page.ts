...
import SwastikLayout from './layout'
import type {
  DynamicFolder,
  DynamicMeta,
...
export function setupSwastikPage({
  pageNextRoute,
  pageOpts,
  swastikLayout,
  themeConfig,
  ...
  if (typeof window === 'undefined') {
    globalThis.__swastik_resolvePageMap = async () => {
      ...
      const clonedPageMap: PageMapItem[] = JSON.parse(
        JSON.stringify(__swastik_internal__.pageMap)
      )
  ...
  if (pageOpts.pageMap) {
    __swastik_internal__.pageMap = pageOpts.pageMap
    __swastik_internal__.Layout = swastikLayout
  } else {
    ...
    pageOpts = {
      ...
      flexsearch: __swastik_internal__.flexsearch
    }
    themeConfig = __swastik_internal__.themeConfig
  }
  ...
  __swastik_internal__.route = pageOpts.route
  __swastik_internal__.context ||= Object.create(null)
  __swastik_internal__.context[pageNextRoute] = {
    Content: MDXContent,
    pageOpts,
    themeConfig
  }
  ...
  if (process.env.NODE_ENV !== 'production' && hot) {
    __swastik_internal__.refreshListeners ||= Object.create(null)
    const checksum = pageOptsChecksum
    hot.data ||= Object.create(null)
    if (hot.data.prevPageOptsChecksum !== checksum) {
      const listeners =
        __swastik_internal__.refreshListeners[pageNextRoute] || []
