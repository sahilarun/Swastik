...
export const parseMeta =
  ...
  tree => {
    visit(tree, ['pre'], preEl => {
      ...
      preEl.__swastik_filename = meta?.match(CODE_BLOCK_FILENAME_REGEX)?.[1]

      preEl.__swastik_hasCopyCode = meta
...
export const attachMeta = () => tree => {
  visit(tree, ['div', 'pre'], node => {
    ...
    node.properties.filename = node.__swastik_filename
    node.properties.hasCopyCode = node.__swastik_hasCopyCode
  })
}
